...
data AST
  ...
  | ASTStr    { astStr  :: String}
  | ASTList   { astList :: [ASTMeta] }
  | ASTBool   { astBool :: Bool }
  | ASTVar    { astVar  :: Name }
  | ASTType   { astType :: RecList Type }
...
data Expr
  ...
  | ExprStr    { exprStr  :: String}
  | ExprDouble { exprDub  :: Double }
  | ExprType   { exprType :: RecList Type }
...
typeOf' :: Expr -> Maybe (RecList Type)
typeOf' ExprInt    {} = pure $ Elem "Int"
typeOf' ExprStr    {} = pure $ Elem "String"
...
-- paramsAndArgs :: [Expr] -> [Expr] -> Maybe ([String], [Expr])
-- paramsAndArgs [] [] = pure ([],[])
