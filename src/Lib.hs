...
-- パースを実行する
execParseFile :: [FilePath] -> IO ()
execParseFile fpaths =
  (\fpath -> do
      program <- readFile fpath
      case pa program of
        Left bundle -> putStrLn (errorBundlePretty bundle)
        Right expr  -> PrettyS.pPrint expr) `mapM_` fpaths
...
-- pa program = parse topLevels "<stdin>" program
pa = parseProgram
